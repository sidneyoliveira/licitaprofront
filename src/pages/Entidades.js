...
const Entidades = () => {
  ...
  // { type: 'entidade'|'orgao', data: {}, parentEntidadeId?: number }
  const [editingItem, setEditingItem] = useState(null);
  // { type: 'entidade'|'orgao', id: number }
  ...
  useEffect(() => {
    ...
    const structuredData = entidades
      ...
      .filter((ent) => {
        if (!term) return true;
        // Busca por nome da entidade OU por nome/código_unidade do órgão relacionado
        const entidadeMatch = ent.nome?.toLowerCase().includes(term);
        const orgaoMatch = ent.orgaos?.some(
          (org) =>
            org.nome?.toLowerCase().includes(term) ||
            (org.codigo_unidade || '').toLowerCase().includes(term)
        );
        return entidadeMatch || orgaoMatch;
      });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-light-text-secondary dark:text-dark-text-secondary">
            Gerencie prefeituras, secretarias e outros órgãos
          </p>
        </div>
        <button
          onClick={() => handleOpenModal(null, 'entidade')}
          className="bg-accent-blue text-white py-2 px-4 rounded-lg flex items-center gap-2"
        >
      ...
      <div className="relative">
        ...
        <input
          type="text"
          placeholder="Buscar por entidade, órgão ou código da unidade..."
          value={searchTerm}
        ...
        {hierarchicalData.map((entidade) => (
          <EntidadeAcordeon
            ...
            onEdit={(orgao = null, type = 'entidade') => {
              if (type === 'orgao') handleOpenModal(orgao, 'orgao', entidade.id);
