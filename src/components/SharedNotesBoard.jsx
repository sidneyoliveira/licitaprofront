...
export default function SharedNotesBoard({
  ...
  return (
    <div className={`bg-white dark:bg-dark-bg-secondary rounded-xl border border-slate-200 dark:border-slate-700 p-4 sm:p-6 ${className}`}>
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-bold text-sm uppercase tracking-wide text-slate-700 dark:text-slate-300">{title}</h3>
      ...
      {showPreferences && prefsOpen && (
        <div className="mb-4 p-3 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/30 space-y-3">
          <button
            type="button"
          ...
          <div>
            <p className="text-xs font-semibold text-slate-500 mb-2 flex items-center gap-1"><Ban className="w-3.5 h-3.5" /> Usuários bloqueados</p>
            ...
            <input
              ...
              className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-dark-bg-primary text-sm"
            />
            {blockSuggestions.length > 0 && (
              <div className="mt-1 border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
                {blockSuggestions.map((u) => (
                  <button key={u.id} type="button" onClick={() => addBlockedUser(u)} className="w-full text-left px-3 py-2 text-sm hover:bg-slate-100 dark:hover:bg-slate-800">
      ...
      <div className="space-y-3 mb-4">
        ...
        <div>
          <p className="text-xs font-semibold text-slate-500 mb-2 flex items-center gap-1"><Users className="w-3.5 h-3.5" /> Compartilhar com usuários</p>
          <div className="flex flex-wrap gap-2 mb-2">
          ...
          {sharingSuggestions.length > 0 && (
            <div className="mt-1 border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
              {sharingSuggestions.map((u) => (
                <button key={u.id} type="button" onClick={() => addShareUser(u)} className="w-full text-left px-3 py-2 text-sm hover:bg-slate-100 dark:hover:bg-slate-800">
                  {u.nome} <span className="text-slate-400">(@{u.username})</span>
                </button>
              ))}
            </div>
      ...
      <div className="space-y-2 max-h-[430px] overflow-y-auto pr-1">
        {loading ? (
          ...
          notes.map((note) => {
            ...
            return (
              <div key={note.id} className="p-3 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-dark-bg-primary">
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    {isEditing ? (
                      <>
                        <input value={editTitle} onChange={(e) => setEditTitle(e.target.value)} className="w-full mb-2 px-2 py-1 rounded border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm" />
                        <textarea value={editText} onChange={(e) => setEditText(e.target.value)} rows={3} className="w-full px-2 py-1 rounded border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm" />
                      </>
                    ) : (
                      <>
