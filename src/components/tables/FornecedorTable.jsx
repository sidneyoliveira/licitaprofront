...
/** Checkbox idêntico ao usado na ItensTable */
...
export default function FornecedorTable({
  ...
  onEdit,
  handleAskDelete,
  // seleção no mesmo padrão da ItensTable (Set de IDs)
  selectedFornecedores,
  onSelectFornecedor,
  onSelectAll,
  areAllSelected,
  // para numerar as linhas conforme a paginação (ex.: (page-1)*pageSize)
  startIndex = 0,
}) {
  return (
      ...
      <table className="w-full divide-y divide-slate-200 dark:divide-slate-700">
        <thead className="bg-slate-50 dark:bg-slate-800/40">
          <tr>
            ...
            <th className="py-3 px-2 text-left text-xs font-extrabold uppercase tracking-wide text-slate-600 dark:text-slate-300">
              #
            </th>
        ...
        <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
          {fornecedores.map((forn, idx) => (
            <tr
              key={forn.id}
              className={`${
                selectedFornecedores?.has?.(forn.id)
            ...
            >
              <td className="py-4 px-4">
                <StyledCheckbox
                  checked={selectedFornecedores?.has?.(forn.id)}
                  onChange={() => onSelectFornecedor?.(forn.id)}
                />
              </td>
              <td className="py-4 px-2 text-sm text-slate-800 dark:text-slate-200">
                {startIndex + idx + 1}
              </td>
              <td className="py-4 px-4 text-sm text-slate-800 dark:text-slate-200">
                {forn.razao_social}
              </td>
              <td className="py-4 px-4 text-sm text-slate-800 dark:text-slate-200">
                {forn.cnpj}
      ...
      {fornecedores.length === 0 && (
        <div className="flex flex-col items-center justify-center p-10 text-center bg-white dark:bg-dark-bg-secondary">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Clique em “Adicionar Fornecedor” para cadastrar/vincular um.
          </p>
        </div>
      )}
    </div>
